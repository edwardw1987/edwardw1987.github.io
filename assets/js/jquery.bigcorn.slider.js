...
(function ($) {
    function BigCornSlider(config) {
        var $this = this,
            ...
            _initOnDirectionLeft: function _initOnDirectionLeft() {
                var self = banner;
                self.imageSlot.css("left", -self.imageSlot.width());
            },
            _initOnDirectionRight: function _initOnDirectionRight() {
                var self = banner;
                self.imageSlot.css("right", -self.imageSlot.width()).children('a').css("float", "right");
            },
            _initOnDirectionBottom: function _initOnDirectionBottom() {
                var self = banner;
                self.imageSlot.css("bottom", -self.imageSlot.height());
            },
            _initOnDirectionTop: function _initOnDirectionTop() {
                var self = banner;
                self.imageSlot.css("top", -self.imageSlot.height());
            },
            _handleImageSlotContent: function _handleImageSlotContent() {
                var self = banner,
                    $imageSlot = self.imageSlot,
                    $buttonSlot = self.buttonSlot;
                if (self.direction == "bottom") {
                    self.images.forEach(function (i) {
                        $imageSlot.prepend("<a href='javascript:;'><img src='" + i + "'></a>");
                    });
                } else {
                    self.images.forEach(function (i) {
                        $imageSlot.append("<a href='javascript:;'><img src='" + i + "'></a>");
                    });
                }
                ...
                */
                ...
                $imageSlot.append($children.eq(0).clone()).prepend($children.eq(-1).clone());
            ...
            _initImageSlotHeight: function _initImageSlotHeight(direction) {
                var self = banner,
                    $imageSlot = self.imageSlot,
            ...
            _initImageSlotWidth: function _initImageSlotWidth(direction) {
            ...
            _registerAnimate: function _registerAnimate(theAnimate) {
                var self = banner,
                    $imageSlot = self.imageSlot;
            ...
            _onClickBtn: function _onClickBtn(callback) {
                var self = banner;
            ...
            _doAnimate: function _doAnimate(config, callback) {
                var self = banner,
                ...
                $imageSlot.animate(_handleConfig(config), self.speed, function () {
                    self.isAnimationRunning = false;
                    callback($imageSlot, curIndex);
                });
            },
            _doAnimateDefault: function _doAnimateDefault() {
                var self = banner;
                self._doAnimate({
                    left: function left($imageSlot, curIndex) {
            ...
            _doAnimateRight: function _doAnimateRight() {
                var self = banner;
                self._doAnimate({
                    right: function right($imageSlot, curIndex) {
            ...
            _doAnimateTop: function _doAnimateTop() {
                var self = banner;
                self._doAnimate({
                    top: function top($imageSlot, curIndex) {
            ...
            _doAnimateBottom: function _doAnimateBottom() {
                var self = banner;
                self._doAnimate({
                    bottom: function bottom($imageSlot, curIndex) {
                        return -($imageSlot.children().height() * (curIndex + 1)) + "px";
                    }
                }, function ($imageSlot, curIndex) {
                    if (curIndex >= self.totalImageCount) {
            ...
            changeButton: function changeButton() {
                var self = banner;
                self.buttonSlot.children().eq(self.curIndex).addClass(self.btnActiveClass).siblings().removeClass(self.btnActiveClass);
            }
        };
        banner.init(config);
    }
